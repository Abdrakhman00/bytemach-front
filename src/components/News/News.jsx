    ...
    this.hotNews = [
      ...
      },
      {
        ...
        avatar:
          "https://www.wallpaperup.com/uploads/wallpapers/2017/12/22/1173395/286d5a20933217acb4066811283dc734-700.jpg",
        text:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Esse hic praesentium sunt incidunt quasi eos? Expedita temporibus dolorum nisi ratione harum quae suscipit, distinctio repellat in! Вero quisquam delectus nemo incidunt commodi perferendis repellendus quaerat, pariatur quidem itaque blanditiis magni.",
        author: "Randi Saputra",
        role: "IT Consultant"
      }
    ];
      ...
      <section id="news" className="container news__container">
        ...
        >
          {this.hotNews.map((feed, index) => (
            <SwiperSlide key={index}> {/* Add key to SwiperSlide */}
              <HotNews
                title={feed.title}
                text={feed.text}
                avatar={feed.avatar}
        ...
        <div className="news__latest">
          ...
          {this.lastestNews.map((feed, index) => (
            <LatestNews
              key={index} // Add key to LatestNews component
              img={feed.img}
              date={feed.date}
              author={feed.author}
              text={feed.text}
